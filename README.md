...
| **codex-delegate** | Codex CLI（`codex exec`）に明示モデル・固定したsandboxと承認ポリシー（`never`）で実装・調査・レビューを委譲。付属ヘルパーが設定の保存と再指定、ターンごとの証拠、rolloutによる実効設定と使用量の照合、スナップショット比較、中断時の残存プロセス処理を担う。macOS/Linux/WSL と Windows の Git Bash で動く |
...
| **defense-driven-refit** | AI生成の設計・コードを説明できる形に改修。採用理由と適合の証拠を分けた台帳で、質問・修正・再検証を回し、未確認事項と見直し条件を明示 |
...
| **gauntlet-loop** | 実在の参照に負けている限り、毎回新しい批評役がブラインド比較で最大の症状を一つ返し、作る役が計測してから直す長時間ループ。参照の梯子・機械の床・停止条件を設計し、貼り付け用の実行プロンプトを出す |
...
| **specprobe** | 仕様・PRD・API契約から重大な曖昧さ・未決の決定・矛盾・検証不能な受入条件を発見し、decision probe と証拠（引用・反例・形式モデル）で示す |
