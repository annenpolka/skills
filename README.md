...
| **codex-delegate** | Codex CLI（`codex exec`）に明示モデル・固定したsandboxと承認ポリシー（`never`）で実装・調査・レビューを委譲。付属ヘルパーが設定の保存と再指定、ターンごとの証拠、rolloutによる実効設定と使用量の照合、スナップショット比較、中断時の残存プロセス処理を担う |
