...
export function validateImageRequest(input) {
  exactKeys(input, ["schemaVersion", "prompt"], ["aspectRatio", "timeoutMs"], "image request");
  ...
  return { ...input, aspectRatio, timeoutMs };
}
...
export class ImageAudit {
  constructor(aspectRatio) { this.aspectRatio = aspectRatio; this.call = null; this.error = null; this.count = 0; }
  observe(update) {
    ...
    if (update.sessionUpdate === "tool_call" && !native) fail("image_audit", "Native image_gen identity is required");
    if (native && (native.version !== 1 || native.namespace !== "grok_build" || native.name !== "image_gen"
    ...
    if (input !== undefined) {
      try { exactKeys(input, ["prompt", "aspect_ratio"], ["variant"], "image tool input"); }
      ...
      if (typeof input.prompt !== "string" || !input.prompt.trim() || input.aspect_ratio !== this.aspectRatio
        || (input.variant !== undefined && input.variant !== "ImageGen")) fail("image_audit", "Unexpected image tool input");
      const normalized = stableJson({ prompt: input.prompt, aspect_ratio: input.aspect_ratio });
    ...
    if (!this.call.input || update.rawOutput?.type !== "ImageGen" || typeof update.rawOutput.path !== "string") {
  ...
  result() {
    if (!this.call?.terminal || !this.call.output) fail("no_image", "No completed image_gen artifact was received");
...
export async function generateImage(request, { outputDir, grokBin = "grok" }) {
  ...
  let peer, sessionId, authSource, timer, interruptHandler, active = false;
  const audit = new ImageAudit(input.aspectRatio);
  let result;
  ...
  try {
    ...
    args.unshift("--tools", "image_gen", "--max-turns", "2");
