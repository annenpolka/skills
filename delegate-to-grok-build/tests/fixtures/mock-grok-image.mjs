...
const native = { version: 1, namespace: "grok_build", name: "image_gen", kind: "image_gen", label: "Generate Image", read_only: false };
...
for await (const line of lines) {
  ...
  if (method === "initialize") send({ id, result: { protocolVersion: 1, authMethods: [{ id: "cached_token" }, { id: "xai.api_key" }] } });
    ...
    const rawInput = { prompt: brief.prompt, aspect_ratio: brief.aspect_ratio };
    update({ sessionUpdate: "tool_call", toolCallId: "image-1", rawInput, _meta: { "x.ai/tool": scenario === "wrong_tool" ? { ...native, name: "bash", kind: "execute" } : native } });
    if (scenario === "multiple") update({ sessionUpdate: "tool_call", toolCallId: "image-2", rawInput, _meta: { "x.ai/tool": native } });
    update({ sessionUpdate: "tool_call_update", toolCallId: "image-1", kind: "other", rawInput: { ...rawInput, variant: "ImageGen", ...(scenario === "changed_input" ? { prompt: "changed" } : {}) } });
    if (scenario === "failed") {
    ...
    const rawOutput = scenario === "no_image" ? { type: "Text", text: "mock-secret-do-not-publish: subscription unavailable" }
      : { type: "ImageGen", path: scenario === "outside_path" ? path.join(process.cwd(), "other.png") : imagePath, filename: "1.jpg", session_folder: "images" };
