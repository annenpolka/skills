...
if [ "$expected_retros" -gt "$actual_retros" ]; then
  ...
  # Collect the actions that need retrospective
  recent=$(sed -n "${start_seq},${end_seq}p" "$LOG_FILE" | jq -r '"  #\(.seq) [\(.tool)] \(.input_summary[:60])"' 2>/dev/null || echo "  (actions ${start_seq}-${end_seq})")
